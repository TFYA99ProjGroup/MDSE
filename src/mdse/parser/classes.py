...
class SymmetryData(httk.HttkObject):
    ...
    @httk.httk_typed_init({'defect_key': int, 'computation': httk.Computation, 'charge': str, 'spin': float, 'workflow': str, 'state': str, 'name': str, 'point_group': str, 'principal_axis_x': float, 'principal_axis_y': float, 'principal_axis_z': float}, index=['defect_key', 'workflow', 'state'])
    def __init__(self, defect_key, computation, charge, spin, workflow, state, name, point_group, principal_axis_x, principal_axis_y, principal_axis_z):
        self.defect_key = defect_key
        self.computation = computation
        self.charge = charge
        self.spin = spin
        ...
        self.principal_axis_x = principal_axis_x # Saved in fractional lattice coordinates
...
class SymmetryBandData(httk.HttkObject):
    ...
    @httk.httk_typed_init({'defect_key': int, 'computation': httk.Computation, 'charge': str, 'spin': float, 'workflow': str, 'state': str, 'name': str, 'irrep_spin1': [str], 'csm_spin1': [str], 'index_spin1': [int], 'center_x_spin1': [float], 'center_y_spin1': [float], 'center_z_spin1': [float], 'irrep_spin2': [str], 'csm_spin2': [str], 'index_spin2': [int], 'center_x_spin2': [float], 'center_y_spin2': [float], 'center_z_spin2': [float]}, index=['defect_key', 'workflow', 'state'])
    def __init__(self, defect_key, computation, charge, spin, workflow, state, name, irrep_spin1, csm_spin1, index_spin1, center_x_spin1, center_y_spin1, center_z_spin1, irrep_spin2, csm_spin2, index_spin2, center_x_spin2, center_y_spin2, center_z_spin2):
        self.defect_key = defect_key
        self.computation = computation
        self.charge = charge
        self.spin = spin
...
class SymmetryTransitionData(httk.HttkObject):
    ...
    @httk.httk_typed_init({'defect_key': int, 'computation': httk.Computation, 'charge': str, 'spin': float, 'workflow': str, 'state': str, 'name': str, 'spin_channel': [int], 'band_from': [str], 'band_to': [str], 'polarization_wrt_principal_axis': [str], 'basis_function': [str], 'basis_function_irrep': [str], 'tdm_character': [str], 'tdm_irrep': [str], 'allowed': [bool]}, index=['defect_key', 'workflow', 'state'])
    def __init__(self, defect_key, computation, charge, spin, workflow, state, name, spin_channel, band_from, band_to, polarization_wrt_principal_axis, basis_function, basis_function_irrep, tdm_character, tdm_irrep, allowed):
        self.defect_key = defect_key
        self.computation = computation
        self.charge = charge
        self.spin = spin
        ...
        self.polarization_wrt_principal_axis = polarization_wrt_principal_axis # Parallel, perpendicular, unpolarized
